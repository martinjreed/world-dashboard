...
def editor_actions(n_save, n_save_bak, n_save_rel, text):
    ...
    if action == "ed-save-reload":
        try:
            ...
            return f"❌ Reload failed (module load from {USER_FILE}): {e}", no_update

        try:
            global df_current
            df_current = make_derived_metrics(df_base.copy(), SH)
        except Exception as e:
            return f"❌ Reload failed (recompute): {e}", no_update

        return f"Reloaded override ✓ {syntax_msg}", {"ts": time.time()}
